...
//주기적으로 플레이어 근처에 아이템을 생성하는 스크립트
public class ItmeSpawner : MonoBehaviour
{
    public GameObject[] itmes; //생성할 아이템
    public Transform playerTransform;//플레이어 트렌스폼

    public float maxDistance = 5f; //플레이어 위치에서 아이템이 배치될 최대 반경

    public float timeBetSpawnMax = 7f; //최대 시간 간격
    public float timeBetSpawnMin = 2f; //최소 시간 간격
    private float timeBetSpawn;// 생성간격
    ...
    private void Spawn()
    {
        ...
        GameObject selectedItem = itmes[Random.Range(0, itmes.Length)];
        GameObject itme = Instantiate(selectedItem, spawnPosition, Quaternion.identity);
